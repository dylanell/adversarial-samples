...
def sillhouette_coefficient(x, y):
    '''
    ...
    class_wise_x_dist_matrices = [
        x_dist_matrix[y == l][:, y == l] \
    ...
    mean_class_wise_rep_dists = torch.cat([
        torch.mean(class_wise_x_dist_matrices[j][torch.triu( \
        torch.ones(class_wise_x_dist_matrices[j].shape[0], \
    ...
    class_clusters = torch.cat([
        torch.mean(x[y == l], dim=0, \
        keepdim=True) for j, l in enumerate(unique_labels)], dim=0)
    ...
    for j, l in enumerate(unique_labels):
        test_points = x[y == l]
        nearest_points = x[
            y == unique_labels[nearest_cluster[j]]]
        n = test_points.shape[0]
        m = nearest_points.shape[0]
        d = test_points.shape[1]
        a = test_points.unsqueeze(-1).repeat(1, 1, m)
        b = torch.transpose(
            nearest_points.unsqueeze(-1).repeat(1, 1, n),
            dim0=2, dim1=0)
