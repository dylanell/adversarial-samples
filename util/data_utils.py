...
def make_gif_from_numbered_images(wildcard_str, dest_path='/tmp/nice.gif'):
    """
    Description: Given a wildcard string to match all image files within a
        directory (e.g. '/tmp/mymodel*.png'), construct a gif from all files
        matching this wildcard. Requires image file names end with an order
        number (e.g. 'image_<order_number>.png') and gif is constructed in
        ascending order by order number. Saves gif to /tmp directory by
        default.
    Args:
        - wildcard_str (string): wildcard string to match all image files to
        make gif.
...
def tile_images(imgs):
    """
    Description: Given a batch of images (must be a perfect square number of
        samples e.g. 64), organize them into a larger tiled image.
    Args:
    ...
    # scale pixel values to [0, 255] and cast to unsigned 8-bit integers
    # (common image datatype)
    min_data, max_data = [float(np.min(imgs)), float(np.max(imgs))]
    min_scale, max_scale = [0., 255.]
    imgs = ((max_scale - min_scale) * (imgs - min_data)
            / (max_data - min_data)) + min_scale
    ...
    for t_idx in range(n_dim):
        ...
        tile_imgs[n_idx:(n_idx + h_dim), m_idx:(m_idx + w_dim), :] \
            = imgs[t_idx]
...
def graph_inputs_with_predictions(
    ...
    # remap inputs to [0 - 255] and convert to uint 8
    new_min, new_max = 0, 255
    min_val, max_val = np.min(inputs), np.max(inputs)
    ...
    for i in range(inputs.shape[0]):
        ...
        axs[i, 0].imshow(img)
        if label_dict:
            axs[i, 0].set_title('Target: {}, Prediction: {}'.format(
                label_dict[labels[i]], label_dict[output_preds[i]]))
        else:
            axs[i, 0].set_title('Target: {}, Prediction: {}'.format(
                labels[i], output_preds[i]))
        axs[i, 0].axis('off')
